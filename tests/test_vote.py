...
def _create_post(authorized_client):
    res = authorized_client.post(
        "/posts/", json={"title": "vote", "content": "body", "published": True}
    )
...
def test_vote_duplicate_conflict(authorized_client):
    post_id = _create_post(authorized_client)
    r1 = authorized_client.post("/vote/", json={"post_id": post_id, "dir": 1})
    assert r1.status_code == status.HTTP_201_CREATED
    r2 = authorized_client.post("/vote/", json={"post_id": post_id, "dir": 1})
...
def test_unvote_success(authorized_client):
    ...
    r1 = authorized_client.post("/vote/", json={"post_id": post_id, "dir": 1})
    assert r1.status_code == status.HTTP_201_CREATED
    r2 = authorized_client.post("/vote/", json={"post_id": post_id, "dir": 0})
